...
@Component({
  selector: 'app-needpopup',
  templateUrl: './needpopup.component.html',
  styleUrls: ['./needpopup.component.css'],
})
export class NeedpopupComponent implements OnInit {
  editdata: any;
  sessionUsername: any;
  selectionHome = false;
  selectionFood = false;
  ...
  constructor(
    ...
    private dialog: MatDialogRef<NeedpopupComponent>
  ) {
  ...
  PostHomeNeed() {
    if (this.homeForm.valid) {
      ...
      this.homeForm.value.homeForm1.need = 'home';
      ...
      this.service
        .updateHomeNeed(this.homeForm.value.homeForm2)
        .subscribe((res) => {});
      this.service.PostNeed(this.homeForm.value.homeForm1).subscribe((res) => {
        this.toastr.success('Need Created Successfully');
        this.dialog.close();
      });
    } else {
  ...
  PostMoneyNeed() {
    if (this.moneyForm.valid) {
      ...
      this.moneyForm.value.moneyForm1.need = 'money';
      ...
      this.service
        .updateMoneyNeed(this.moneyForm.value.moneyForm2)
        .subscribe((res) => {});
      this.service
        .PostNeed(this.moneyForm.value.moneyForm1)
        .subscribe((res) => {
          this.toastr.success('Need Created Successfully');
          this.dialog.close();
        });
  ...
  PostFoodNeed() {
    if (this.foodForm.valid) {
      ...
      this.foodForm.value.foodForm1.need = 'food';
      ...
      this.service
        .updateFoodNeed(this.foodForm.value.foodForm2)
        .subscribe((res) => {});
      this.service.PostNeed(this.foodForm.value.foodForm1).subscribe((res) => {
        this.toastr.success('Need Created Successfully');
        this.dialog.close();
      });
    } else {
